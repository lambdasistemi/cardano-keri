...
/-!
# Mirror statements M1 … M11 — unproven
...
theorem M2_revocation_is_permanent (p : Params) (env : TelEnv) {s s' : Sys} {a : Action}
...
same system land in either order on the same system: the set converges
however it is filled, which is why the mirror needs no completeness
guarantee. Mutant: a push that records its position (an order-dependent
accumulator). -/
theorem M4_pushes_commute (p : Params) (env : TelEnv) {s s₁ s₂ : Sys} {w₁ w₂ : Walk}
    (hne : w₁.tel.i ≠ w₂.tel.i)
...
/-- **M7. Open, exactly** (public inversion). Mutant: `open` accepting an
`iss` as the inception. -/
theorem M7_open_iff (p : Params) (env : TelEnv) (s : Sys) (issuer : AID) (w : Walk) (s' : Sys) :
    stepFn p env s (.open issuer w) = some s' ↔
      ∃ c, s.reg w.tel.i = none ∧ s.ckpt issuer = some c ∧ w.tel.kind = .vcp ∧ w.tel.ri = w.tel.i ∧
        (∃ v, sealWalk p env c w.tel.i w = some v) ∧
        s' = s.setReg w.tel.i (some ⟨issuer, w.tel.i, fun _ => false⟩) := by
  sorry

/-- **M8. The mirror never reads current keys.** Opening and pushing are
invariant under the issuer's current key state (S3 lifted to the system):
"current keys cannot vouch for past events" and cannot block them either.
Mutant: `push` requiring `env.signed c.cur`. -/
theorem M8_mirror_ignores_current_keys (p : Params) (env : TelEnv) (s : Sys) (aid : AID) (x : Epoch)
    (a : Action) (hmirror : (∃ i w, a = .open i w) ∨ ∃ w, a = .push w) :
