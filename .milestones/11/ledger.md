...
## Work deleted 2026-09-07 — the record is not

On the operator's instruction, this retired milestone's implementation work was
removed: pull requests #305 and #306 closed, and every branch deleted, local
and remote — `ms11/r1-event-key-v2`, `ms11/s0-reland`, `ms11/s2-no-b` and
`docs/300-projection-fidelity-requirements` on origin, plus fifteen local
`ms11/*` branches.

**Every deleted tip is recorded in `deleted-refs-2026-09-07.txt` beside this
file**, so any branch is restorable with `git branch <name> <sha>` while those
objects survive in a clone. The milestone had no open issues.

This page, its design notes, mandates, recut and watcher preparation, and the
history file are **preserved unchanged**. The record outranks the
implementation: code is regenerable from a good record, and the record is not
regenerable from code. What was deleted is the code; what M1.2 learned stays
here.

Deleted by the M1 desk because the branches lived in the shared `cardano-keri`
repository and made the pull-request board misreport what was being worked on.
The milestone's retirement itself remains project ruling D-021, 2026-09-02.
