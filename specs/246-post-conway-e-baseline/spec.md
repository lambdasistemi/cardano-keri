...
| R-11 | Every aggregate a record carries publishes its denominator. A completeness result reports how many declared artifacts were **examined** beside how many were **missing**; `0 missing` is meaningless without `N declared`. Where the denominator cannot be established — inventory absent, unreadable, or unexpectedly empty — the result is `MEASUREMENT-FAILED`, which is `COULD-NOT-EVALUATE` and therefore RED, never a zero. |
| R-12 | Every measured quantity carries its **instrument** and its **measurement window**. A number without the thing that produced it and the interval it covers is not a measurement. A percentage without its reset window is the standing example. Where the window cannot be stated, the quantity is `COULD-NOT-EVALUATE`, not a smaller number. |
