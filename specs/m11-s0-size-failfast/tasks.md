...
- [ ] **S0-T06** Recompute and record the pinned Aiken binary digest/version at
  measurement time under the build-token contract.
- [ ] **S0-T07** Build under a pseudo-TTY and capture raw output plus exit code.
- [ ] **S0-T08** Emit each valid bare verdict immediately with bytes, both
  percentages/headrooms, threshold verdict, and transaction-fit caveat.
- [ ] **S0-T09** Commit the deterministic seven-row report and reproducibility
  harness with source/blueprint/toolchain evidence identities.

## S0-P4 — independent acceptance

- [ ] **S0-T10** Obtain a fresh alternate-family audit of the exact candidate
