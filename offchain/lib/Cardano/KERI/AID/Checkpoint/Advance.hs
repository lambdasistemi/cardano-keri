...
Controller signatures ('advancePredicate') verify over the KERI
@rot@ event's own bytes ('aeEventBytes') at distinct positions of
@cdCurKeys new@ (V5, #219: mirrors registration's R7 — no
reconstructed Cardano-domain message preimage); the AE1-AE10
event-binding slice set pins the attested KERI @rot@ event bytes to
the created state (V6, reusing "Cardano.KERI.AID.Checkpoint.Registration"'s
@respellHex@\/@respellThreshold@\/@qb64WitnessVerkey@); and the
incoming-set witness receipt gate (V7) counts distinct valid receipts
indexed into the created datum's own (already W3-derived and
eq7-pinned) witness list. No SAID\/blake3 proof is taken over the rot
bytes (ratified QC): @d@ and @p@ spans are deliberately unchecked.
-}
module Cardano.KERI.AID.Checkpoint.Advance (
    -- * Advance evidence
    AdvanceEvidence (..),
...
'aeCtrlSigs' index into @NEW.cur_keys@ and sign 'aeEventBytes' directly
(#219: mirrors registration's R7 — no reconstructed Cardano-domain
message preimage). 'aeWitReceipts' index into the __derived__ incoming
...
key-state fields, and the evidence's witness-delta lists.

Retained only for 'Cardano.KERI.Deployment.Advance.mkAdvancePackage'
(offchain\/deployment, out of #219 phase-1 scope) — dead to
'advancePredicate' since #219, which verifies 'aeCtrlSigs' against
'aeEventBytes' directly. Tracked fast-follow deletion after #181.
-}
reconstructAdvanceMessage ::
    SpentCheckpoint ->
...
{- | The pure advance predicate: V4 (direct @spent@-vs-@new@ equalities,
...
signature verifies over 'aeEventBytes' directly (#219) against
@NEW.cur_keys@ at that position (bad index or bad signature never
counts, never aborts — the @verified_positions@ convention).
