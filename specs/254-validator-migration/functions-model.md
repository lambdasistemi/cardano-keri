# Functions model — #254 validator-version migration
...
- **Arguments:** `source` carries network/version/policy/outref/role/state;
  `target` carries version/policy/address/role/refund.
...
- **Requirement / slice:** RQ-254-01, RQ-254-02, RQ-254-09 / S254-1.
- **Signature:** `valid_migration_edge(source: MigrationSource, successor: MigrationSuccessor, expected_predecessor: Predecessor) -> MigrationVerdict`
- **Arguments:** actual consumed source, actual proposed successor, applied
  predecessor parameter.
- **Result:** structured version/origin/continuity verdict.
- **Constraints:** N to N+1; exact source policy/ref; target marker equals
  applied version.
...
- **Signature:** `validate_checkpoint_migrate_out(version: Int, authorization: MigrationAuthorization, own_ref: OutputReference, tx: Transaction) -> Bool`
- **Arguments:** applied source version, authorization, named input, full
  transaction.
...
- **Signature:** `validate_checkpoint_migrate_in(version: Int, predecessor: Predecessor, source_ref: OutputReference, authorization: MigrationAuthorization, policy_id: PolicyId, tx: Transaction) -> Bool`
- **Arguments:** applied target version, pinned predecessor, source ref,
  authorization, target policy, transaction.
- **Result:** ledger acceptance.
- **Constraints:** verifies the old arm ran, predecessor is genuine, one old
  burn/one new mint, origin and target output are exact.
...
- **Constraints:** preprod network/policy and ACTIVE only; proves v0
  Close/CloseBurn, exact refund, equal successor capitalization, and origin.
...
- **Constraints:** all KEL fields and role payload exact; only version/origin
  and source-to-target policy token change.
...
- **Signature:** `validate_board_migrate_out(version: Int, target: MigrationTarget, own_ref: OutputReference, tx: Transaction) -> Bool`
- **Arguments:** applied version, authorized target, board input, transaction.
- **Result:** ledger acceptance.
...
- **Signature:** `validate_board_migrate_in(version: Int, predecessor: Predecessor, source_ref: OutputReference, target_record: VersionedBoardDatum, policy_id: PolicyId, tx: Transaction) -> Bool`
- **Arguments:** target version/predecessor, source ref, proposed target datum,
...
- **Signature:** `validate_legacy_board_migrate_in(preprod_v0: LegacyBoardIdentity, source_ref: OutputReference, target_record: VersionedBoardDatum, policy_id: PolicyId, tx: Transaction) -> Bool`
- **Arguments:** frozen v0 identity, source record, target datum/policy,
  transaction.
- **Result:** ledger acceptance.
- **Constraints:** proves old Retire/Burn and owner signer, target
  authentication, same witness/content/owner, deposit, marker, and origin.
...
- **Arguments:** source/target/version/role/value/signature context and
  proposed transition.
...
- **Constraints:** exact identities, unique/contiguous versions, valid
  predecessor edges, historical retention, earliest scan point.
...
- **Signature:** `prepare_checkpoint_migration(registry: ValidatedRegistry, source: ActiveCheckpoint, target_version: ValidatorVersion, refund_address: Address) -> Either MigrationPlanError CheckpointMigrationPackage`
- **Arguments:** registry, exact source row, target, legacy refund.
- **Result:** unsigned canonical package containing no secret material.
- **Constraints:** family/role/version selected from registry; v0 non-ACTIVE
