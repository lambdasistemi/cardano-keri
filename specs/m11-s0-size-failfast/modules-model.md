...
| S0-M02 | append validator | Represent append-only record mutation | Decode raw event/parsed token, bind event identity, verify an authenticated-map proof/update, derive a changed root/occupancy result |
| S0-M03 | cursor validator | Represent validator-derived cursor movement | Consume parsed transition data plus historical proof, compute changed keys/state/grade/slot fields, compare old/new cursor |
...
| S0-M06 | staging/proof-token validator | Represent the parsing stage | Total-parse event bytes through the inherited bytes-only parser surface, bind SAID/derived fields, bind proof-token identity/output |
