...
| Capability | Status |
|---|---|
| Seq-0 binding verifiable from KEL | Native Blake3 AIDs: genesis remains registration-attested pending a full-context single-tx measurement (identity-model §7a; lane-packed spike #88 core fits the whole single-chunk domain, 55.1% cpu / 75.2% mem at 1024 bytes); F-prefix AIDs (CF sidecar): on-chain self-cert |
