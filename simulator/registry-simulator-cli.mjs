#!/usr/bin/env node
/*
 * registry-simulator-cli.mjs — Node adapter over the registry backend.
 * Owns argv, stdout/stderr and exit status. Tree and machine semantics stay
 * in registry-simulator-backend.mjs.
 *
 *   node simulator/registry-simulator-cli.mjs --story 1 --to-end --json
 *   node simulator/registry-simulator-cli.mjs --story 1 --fork mallory-go --to-end --json
 */
...
import {
  ...
  selectPath,
  projectJson,
} from './registry-simulator-backend.mjs';

function usage() {
  return 'usage: registry-simulator-cli.mjs --story N [--fork ID] [--to-end] [--json]';
}
...
/**
 * Run the registry simulator CLI.
