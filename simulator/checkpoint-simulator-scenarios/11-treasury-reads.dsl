...
story: 11
title: "The treasury reads Alice's checkpoint"
goal: "As the treasury validator, I want to authorize a payment only if Alice's keys really are current, so I read her checkpoint as a reference input."
params:
  D: 1000
  B: 5
  P: 2
  W: 10
atoms: ["11.present", "11.the-freeze-bond-held", "11.not-poisoned", "11.born-at-least-w-slots", "11.the-signature-on-the-payment", "11.later-when-validity-ships-within", "11.on-absent", "11.frozen", "11.poisoned", "11.juvenile", "11.parked", "11.convicted"]
step:
  slot: 0
  who: treasury
  say: "Absent: the treasury fails closed."
  expect:
    verdict: not-present
step:
  slot: 0
  who: alice
  say: "Registered: juvenile for W slots."
  action:
    register:
      refund: 1
      pool0: 1
...
step:
  slot: 10
  who: treasury
  say: "Born at 0, W = 10: consumable at slot 10."
  expect:
    verdict: consumable
step:
  slot: 12
  who: hal
  say: "Frozen: the freeze bond is missing."
  evidence:
...
step:
  slot: 15
  who: friend
  say: "A sponsor tops up the frozen checkpoint: the pool grows, the checkpoint stays frozen. Nothing but Alice’s next keys can make it consumable again."
  action:
    topUp:
      x: 5
  expect:
    ok: true
    verdict: frozen
    exhibits: [T10_bonds_are_observable, T10_inert_without_next_keys, T14_pool_increases_only_by_topup, T1_sn_monotone, T1_sn_monotone_all, T1_trace_sn_monotone, T3_epoch_local, T6_component_conservation, T6_dreg_never_a_fee, T6_dreg_never_moves_between_present_states, T7_step_iff_stepFn, T7_trace_iff_replay, T8_edges_leave_the_leaf, T8_leaf_agrees_with_state, T8_leaf_never_absent_again, T8_leaf_states, T8_only_convicted_is_terminal, T8_present_implies_registered, T8_sysstep_partition, T8_utxo_iff_active, T9_juvenility_is_consumer_only, consumableStateB_iff, trace_poison_fold, trace_sn_monotone_all]
step:
  slot: 20
  who: alice
  say: "Unfrozen by a depositing rotation: consumable at once (juvenility is untouched by a deposit)."
  evidence:
    add: [{"intentAuthorized":[1,"deposit",null]}]
  action:
    rotate:
      sn': 1
      op: deposit
...
step:
  slot: 30
  who: alice
  say: "Parked by the reap: no checkpoint to read."
  evidence:
    add: [{"rotationTo":[1,1,2]},{"intentAuthorized":[2,{"close":{"payee":1}},null]}]
  action:
    close:
      sn': 2
      payee: 1
...
step:
  slot: 40
  who: alice
  say: "Revived through the registry with a witnessed rotation from the parked key state: juvenile."
  evidence:
    add: [{"rotationTo":[2,2,3]}]
  action:
    reopen:
      sn': 3
      refund: 1
...
step:
  slot: 50
  who: treasury
  say: "Consumable. What the treasury cannot know: whether Alice rotated on KERI an hour ago and no hunter has landed it yet. That is why the pool exists."
  expect:
    verdict: consumable
step:
  slot: 50
  who: cora
  say: "Convicted: fails closed, forever."
  evidence:
    add: [{"duplicityAt":[3,3]}]
  action:
    convict:
  ...
  expect:
    ok: true
    verdict: not-present
    exhibits: [T10_bonds_are_observable, T10_parked_holds_nothing, T12_convict_exact, T16_payments_are_named, T4_current_key_thief_cannot_park, T5_convict_enabled, T6_component_conservation, T6_dreg_never_a_fee, T7_step_iff_stepFn, T7_trace_iff_replay, T8_edges_leave_the_leaf, T8_leaf_agrees_with_state, T8_leaf_never_absent_again, T8_leaf_states, T8_present_implies_registered, T8_reopen_actor_is_proof, T8_sysstep_partition, T8_utxo_iff_active, T9_juvenility_is_consumer_only, consumableStateB_iff]
