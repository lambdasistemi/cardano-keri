...
contract:   pr-descriptions-are-for-the-reader (PUBLIC REPO)
parties:    every PR author in lambdasistemi/cardano-keri ⟷ the operator, and
            every stranger reading a PUBLIC repository
invariant:  a PR description states WHAT CHANGED, WHY, HOW TO VERIFY (from a
            clean checkout, or a CI run) and its LIMITS — every claim
            checkable by a reader who has only the repository. NO internal
            process: no roles, providers, models, skills, pins/hashes,
            mandates, campaigns, budgets, invariant-row counts, tail-stops or
            host rules; NO untracked or machine-local paths (./gate.sh,
            /tmp/...). Facts in past tense; residuals named plainly.
enforced:   BY TEMPLATE — `.github/pull_request_template.md`, installed by
            #275, which enforces the standard at the moment of writing rather
            than by a later sweep. Retroactive application is #275's scrub of
            96 scanned PR bodies.
            Trigger: operator saw PR #273 publish a skill-pin table with
            hashes, orchestration ceremony, a host GPG rule and a FALSE
            "CI is dead host-wide" claim, on a repo verified PUBLIC (gh repo
            view → PUBLIC) where merged bodies stay world-readable.
            DESK RULING on the 23 `./gate.sh` citations: REPHRASE, DO NOT
            DELETE — an untracked script cited as acceptance evidence is not
            merely a process leak, it is UNVERIFIABLE EVIDENCE, the exact
            failure class this milestone exists to eliminate.

contract:   census-milestone ⟷ M1 priority and disclosure (PENDING SEAT)
parties:    the money-path census milestone (pre-authorized, unseated) ⟷ M1;
            and both ⟷ the operator, who owns the disclosure ruling
invariant:  (a) PRIORITY — a census class rated pre-M1 PREEMPTS M1 sequencing
            by default; M1 absorbs the reorder without per-finding
            negotiation; a disputed rating escalates to the machine, never
            settled bilaterally. (b) DISCLOSURE — no census finding is filed
            ANYWHERE until the operator rules the channel; if the seat is
            released first, the lane may census and HOLD, but may not file.
enforced:   (a) ACCEPTED AND RECORDED as M1's own commitment (machine, 2026-08-13),
            given before M1 knew which of its work it would cost.
            (b) RULED BY THE OPERATOR 2026-08-13: **FILE FINDINGS PUBLICLY.**
            The hold is LIFTED and PVR is NOT to be enabled — it buys nothing
            and adds a channel nobody reads. THE PREMISE WAS THE DEFECT, not
            the facts: no mainnet, no announced preprod, no users, no funds,
            nothing running — so a public finding today is a bug report on
            unlaunched code nobody operates, and the exploit framing does not
            apply to the current state.
            THE ERROR IS THE ENTRY, and it was made twice: the desk reported
            the public-repo risk and the machine VERIFIED THE FACTS AROUND THE
            CLAIM (private=false, pvr=null, secret_scanning=disabled — all
            true) INSTEAD OF THE CLAIM ITSELF, which is "something exploitable
            has value at stake". A FACT IS NOT EVIDENCE UNLESS IT IS BOUND TO
            THE THING UNDER DECISION — this milestone's own rule, broken by
            the two seats that adopted it, with verification standing in for
            diligence.
            WHAT SURVIVES, RE-TIMED FROM THE CENSUS ENTRY TO M1'S EXIT:
            **no census finding is still open when preprod is announced.**
            Public issues are permanent and indexed and the announcement is
            ahead of us, so the gate belongs where the risk starts. Carried as
            an M1 EXIT CONDITION; the census files freely meanwhile.
            SEAT: pre-authorized, seating DEFERRED on capacity (claude 67%,
            codex-sol 39% against a standing pause order at claude ≥80% OR
            codex ≥40% — one point away). First in the queue when the
            threshold resolves; needs no re-argument. e274's S254-R tree-wide
            controls (arity-parity, version-remnant) carry into its brief as
            prior coverage.

contract:   AF-07 call-site-vacuity class — HANDOVER TO THE CENSUS MILESTONE
parties:    #254 (found it, ships one control) ⟷ the money-path census
            milestone (pre-authorized, unseated) ⟷ every future auditor
invariant:  a guard that exists at one call site does not protect the other
            call sites of the same operation; a suite that does not ENUMERATE
            call sites cannot detect the gap, so the check passes while the
            operation is unguarded
enforced:   PARTIAL, and handed over rather than chased. #254 kills the
            BLOCKING rows and ships its arity-parity + version-remnant
            tree-wide controls; the SURVIVING members leave the ticket as
            NAMED RESIDUALS by desk ruling 2026-08-14 (scope frozen to ship
            the migration).
            KNOWN MEMBERS AT HANDOVER: inventoryByAid (one guard, 2 call
            sites, only successor observed — source-side duplicate-AID attack
            unobserved), plus 5 more found by generalisation probe P4
            including PT-out-signature and COMBO-out-auth — **both AUTH
            PATHS**, which the census must treat as a priority input, not an
            inventory line.
            WHY IT LEAVES #254: the class is REAL, which is exactly why every
            generalisation probe finds more members — an unbounded search
            inside a ticket whose product is a migration. The census exists
            for precisely this: classes that reach code outside the diff.
            IT ARRIVES WITH PRIOR COVERAGE: #254's S254-R tree-wide controls
            (arity-parity, version-remnant) transfer as free coverage, and
            the arity control has already PROVEN itself by catching a real
            cross-ticket integration gap on the merge path (endpoint_board
            declares 1 applied parameter, deployment supplied 0).

contract:   scrutiny-tier — depth is a PROPERTY OF THE CHANGE, fixed at spec time
parties:    every ticket in this milestone ⟷ its owner, its auditor, and the
            desk that would otherwise re-decide depth under pressure
invariant:  the amount of proof a change must carry is decided ONCE, from what
            the change can irreversibly break, BEFORE work starts — never
            per-event, never by whoever is most alarmed that day.

            TIER 1 — IRREVERSIBLE: deployed validators, migrations, anything
              holding real value that cannot be patched after deployment.
              Full audit; declared invariant rows with spec-time severity;
              mutation campaign under its stopping rule; submission 1
              full-scope, submission 2 delta-scoped and FINAL.
            TIER 2 — REVERSIBLE CODE: offchain, CLI, indexer, deployment
              scripts, anything a later PR can fix. **The CI gate IS the
              audit.** Green at a named head = merge. An auditor only if the
              ticket owner asks for one and says why.
            TIER 3 — DOCS, METADATA, TESTS, TOOLING: no audit round. Green =
              merge.

            STABILITY RULES, which are the actual product here:
            - The tier is set at spec time and **may never be RAISED IN PLACE**.
              AMENDED 2026-08-14 on the machine owner.s concern, which was
              right: the absolute form forbade the case that matters most —
              a ticket assigned TIER 2 in good faith whose work REVEALS it
              touches a deployed validator or an unpatchable path, with the
              "re-cut before work" hatch closing exactly when the discovery
              happens. NOT EVERY RAISE IS A RATCHET: the measured ratchet was
              the SAME tier re-justified upward round after round, invisibly.
              A single raise triggered by a NEW FACT about what the change can
              break is a different event. So: **a discovered irreversibility
              STOPS the ticket and RE-CUTS it at the correct tier, with the
              discovery named.** The re-cut is visible and costly, so it
              cannot serve as a ratchet — but it exists, and discovery is
              never unspeakable. Raising it mid-flight is the ratchet, and it is
              how #254 spent 60 hours: tier-1 scrutiny applied to a tier-2
              deployment fix, one defensible grant at a time.
            - The tier MAY be lowered mid-ticket (ship-bias).
            - Nobody raises depth because of an incident in another ticket.
              That finding goes to the census/backlog, not into this ticket's
              scrutiny.
            - Merges are PRE-AUTHORIZED standing for tiers 2 and 3: green at a
              named head → the lane merges, no desk round trip. Not an
              emergency measure — the permanent default.
            - A hold after tier assignment requires ONE named question with a
              binary consequence, not a campaign.
enforced:   BY THE DESK, standing from 2026-08-14, after the operator named
            process INSTABILITY as worse than either extreme: five days of
            unbounded rigour, then an hour of everything-mergeable, then a
            hold. Every swing was locally justified; the aggregate was
            unpredictable. Depth decided by mood is the defect; depth decided
            by irreversibility is the fix.

contract:   grok as a STANDING authoritative family for commit-owner seats
parties:    every new team seated in this milestone ⟷ the shared seating
            contracts (commit-owner, ticket-orchestrator, tmux-orchestrator)
invariant:  new teams include `grok` as an eligible COMMIT-OWNER family, and
            alternation still holds: the seat that disbelieves (auditor) comes
            from a DIFFERENT family than the seat that produced. With three
            families (claude, codex, grok) a grok commit owner is audited by
            claude or codex, and its ticket owner is not grok.
enforced:   OPERATOR RULING 2026-08-14 ("I want new teams to include grok as
            commit-owner"). Precedent already exists: grok %6419 owned
            S254-E submission 2 under an operator-ordered rotation and its
            candidate was audited by codex — alternation held in practice.
            **BLOCKER FOUND BEFORE SEATING, not after**: the shared contracts
            disagree.
            - `tmux-orchestrator` (generalized): standing families are claude
              and codex; another family is a PER-SEAT EXCEPTION citing an
              operator order, "not an addition to the standing family set" —
              and it names grok explicitly as passable only as an
              already-authorized seat.
            - `ticket-orchestrator`: a T.O. not running Codex or Claude must
              be RELAUNCHED into one of those families.
            - `commit-owner:33` (still binary): requires owner_cli to be "the
              alternate authoritative family ... Codex dispatches Claude, and
              Claude dispatches Codex". A grok commit owner reading this
              strictly appends CONTRACT-CHALLENGE and STOPS before touching
              the repo.
            So the operator's rule needs the standing family SET widened in
            shared doctrine, not just a per-seat exception each time —
            otherwise every grok commit owner either self-refuses or proceeds
            on a loose reading of a rule that enumerates two families.
            Raised to the project owner (routing changed 2026-08-14); until it
            lands, grok commit owners are seated as per-seat operator-order
            exceptions with the order cited in brief and START, exactly as
            tmux-orchestrator requires.

contract:   family-allocation — no family carries the board
parties:    every seat in this milestone ⟷ the alternation guarantee
invariant:  the seat that DISBELIEVES comes from a different family than the
            seat that PRODUCED — which one family holding every worker seat
            erodes quietly, because you cannot alternate against yourself.
            Allocation: COMMIT OWNER = grok by default (operator ruling,
            per-seat order cited); AUDITOR = grok is ALREADY eligible
            (commit-auditor:40-43 "authoritative auditor family distinct from
            the commit owner, derived from the current family set" + an
            operator-order reference for a non-standing family) but never in
            the same submission as a grok commit owner; TICKET OWNER = codex
            or claude only (ticket-orchestrator:117 forces relaunch
            otherwise — the one real block, with the project owner);
            CLAUDE reserved for judgement seats, its weekly being the scarce
            bucket; CODEX is NOT the automatic fallback when Claude is
            constrained — grok is.
            Report the family filling each seat in STATUS.
enforced:   OPERATOR 2026-08-14 ("we are abusing codex" / "we should engage
            grok more"). MEASURED CAUSE: live seats were 4 codex / 3 claude,
            and two claude seats were orchestrators — so nearly every WORKER
            seat was codex while grok, authorized for commit-owner the same
            day, held ZERO. Every individual choice was defensible ("Claude's
            weekly is constrained, seat Codex") and FOUR DEFENSIBLE LOCAL
            CHOICES PUT ONE FAMILY ON THE WHOLE BOARD — the ceiling ratchet in
            a different currency. Distribution was being read off
            `tmux list-panes` rather than reported, which is how it drifted
            without anyone deciding it.

contract:   grok-4.6 constrained-auditor PILOT (three trials, milestone-scoped)
parties:    M1's epics ⟷ the machine owner's pilot grant ⟷ the parent ticket
            owners who still hold acceptance
invariant:  a Grok 4.6 auditor is QUALIFYING, not qualified. Eligible as sole
            auditor ONLY for read-only judgement-light work: primary-source /
            citation, spec-versus-code, terminology/actor/trust-boundary,
            dependency/falsifier, provenance. INELIGIBLE as sole auditor for
            feasibility/probability/effort, load-bearing arithmetic, LIVE
            INVENTORIES, cryptographic/security acceptance, and IRREVERSIBLE
            merge or deployment acceptance.
            A trial requires, BEFORE launch, a SEALED parent-authored answer
            key with at least one hidden ARITHMETIC fault and one hidden
            LIVE-STATE fault; each trial is shadow-graded by a fresh Opus 5
            high-effort audit; EXACTLY THREE shadow audits are authorized and
            a failed series needs a new machine-owner grant to retry.
            **An ordinary Grok audit without the pre-sealed planted-fault key
            IS NOT A TRIAL** — it cannot be counted as one before or claimed
            as one after. Acceptance never moves: it stays with the parent
            ticket owner. Only launch: `grok --always-approve -m grok-4.6`.
enforced:   BY THE PILOT GRANT (machine owner, cascaded via the project owner
            2026-08-14, POINTER-1786703163-3932002). M1 cascaded it to e274
            ONLY — e156 holds #162, whose audits are the relayer's live/behavioural
            surface, and spending a scarce trial there is both a poor fit and a
            distraction from a TIER 2 ticket whose gate is already its audit.
            INTERACTION WITH M1'S GROK CAP: one grok seat per ticket, so a
            ticket with a grok commit owner cannot also run a grok trial
            auditor — independently of alternation, which already forbids
            auditor and producer sharing a family.
            NOTE the pilot's exclusions bite exactly where M1's remaining work
            lives: the cutover, #279's preprod inventory and deployed-validator
            acceptance are live inventories and irreversible acceptance —
            named ineligible, so no trial may be spent there.
