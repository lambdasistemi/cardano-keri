...
- [X] T106-S7 keripy generator: add a `fork_witnessed` bundle (witnessed AID; rot_recorded + rot_conflict both carry witness receipts from the tip's witnesses — the collusion/duplicity). **Non-witness-changing: both rotations keep the icp's witness set unchanged** (the receipts must index into `D.witnesses`; witness-changing forks are #24 scope). Regenerate fixtures (hermetic keripy flake); drift-stable.
- [X] T106-S7 **conflict-check `b` fix (framing hole)**: drop the witness term from the convict conflict check — `eeWitnesses e == cdWitnesses d` (Haskell `agrees`) and `evidence.witnesses == tip.witnesses` (Aiken `forward_agrees`). A rotation carries no `b`, so on a witnessed AID the phantom `[] ≠ D.witnesses` fakes a conflict; with Convict 4 that lets a witnessed identity's OWN honest recorded rotation convict it. Conflict axes become `n`/`nt`/`bt` (spec step 5). Add **F3b** = a witnessed AID's own recorded rotation (no `n`/`nt`/`bt` divergence) → `CvNoConflict`, both languages.
- [X] T106-S7 `convictPredicate`/`convict_predicate` gain the Convict-4 witness-receipt check (wit_sigs verify over event_bytes against tip.witnesses, count ≥ tip.toad), both languages; new error `CvInsufficientReceipts`/`CvUnwitnessed`. Existing checks/order preserved otherwise.
- [X] T106-S7 vectors + parity: `fork_witnessed` convict → Valid (witnessed duplicity); the existing witnessless `fork` → still Valid (toad=0 vacuous tier); **F1b** = unwitnessed conflicting event on a witnessed AID (receipts < toad) → `CvInsufficientReceipts` (mutate fork_witnessed); **F3b** = the witnessed honest recorded rotation → `CvNoConflict`. Both languages, verdict parity.
- [X] T106-S7 update MEASUREMENTS.md if the added witness Ed25519 shifts the convict cell (re-measure convict on fork_witnessed); RED→GREEN pair handshake; gate green; committed with trailer `Tasks: T106-S7`.
