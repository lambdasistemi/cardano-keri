...
# 2026-09-14 — the desk died and was resurrected; one merge and one Lean candidate in hand

## What the previous desk did this morning, unrecorded

Desk `%459` acted between 11:00Z and 11:33Z with the operator present, then
its whole tmux session was gone by 11:43Z. Nothing it did reached this ledger;
it is reconstructed here from lane journals, inbox notes and git.

| act | evidence |
|---|---|
| released ticket-387 to rebase, re-gate and merge PR #390 | `ticket-387/inbox/NOTE-012`, journal `RESUMED` 11:01Z |
| ticket-387 rebased onto `8021ff9`, test 1 PASS, gate v9 PASS, pushed `fbb851e` draft | journal `GATE-PASS` 11:32:01Z, `PUSHED` 11:32:37Z |
| released the Lean lane, then withdrew it two minutes later | `epic-lean-invariants/inbox/NOTE-010`, `NOTE-011` |
| committed and pushed four commits on `fix/408-registry-lifecycle` | `17008c4`…`27a7732`, 11:14–11:20Z, PR #441 |

**The ticket-387 seat died one step short of merging.** PR #390 is
MERGEABLE, CLEAN, 21 checks green, still draft. The merge is authorized
(NOTE-012, content-bound) and must be executed by a re-seated ticket-387
owner, not by the desk.

## The Lean candidate, stated as a claim

PR #441 at `27a7732` says, in `lean/CardanoKeri/REGISTRY-408-OBLIGATIONS.md`:
six theorems proved (`parked_leaf_no_ckpt`, `custody_of_pending`, `W1_reach`,
`W3_key_still_pending`, `W4_wrong_recipient_refused`, `W5_tomb_leaf`) with
`CollectAxioms` showing no `sorryAx`; both libraries build with no `sorry`
warning; the trace driver repaired (`Params.W` gone); five mutants all red at
target. A grep of the branch finds no `sorry` in any `.lean` outside doc
comments. CI: 21 checks green, branch BEHIND `main`.

**Unverified by anyone independent.** The A-006/A-007 conditions — a frozen and
falsified actual-interface gate, independent completeness and inversion
inspection, approved axioms — have not run. The obligation record's own row 7
leaves `Cage.lean` consolidation against Singular *undecided*, so the
Singular-import direction announced in NOTE-011 was not executed in these
commits; Singular's Lean was only measured (builds clean on pinned 4.27, no
dependencies).

## Open at the desk

1. Operator: does the NOTE-011 direction (import Singular Lean as registry
   authority) still govern, or does PR #441's in-place repair supersede it?
2. Operator: which lanes are released from the 2026-09-11 pause?
3. Re-seat ticket-387 for the one remaining step on #390.
4. Independent Lean audit of #441 once (1) is answered.
5. Record repair: three `ticket-410` sub-lanes without terminal events.

## Desk identity

New desk pane `%1156`, session renamed `59 → keri`, window
`cardano-keri-ms1-identity-core`, family claude, model `claude-fable-5-1`.
Registry line updated in `llm-settings/shared/milestones.md`.

# 2026-09-15 — paused overnight, released for one task: Singular's Lean as the registry authority

OMNIA PAUSA of 2026-09-14T17:31Z was complied with (PAUSED 17:32:03Z). At
11:08Z the operator released this desk for one task: "resume work on
integrating Singular lean", then "use GLM and or muse". All other lanes stay
paused. Plan v3 is still unruled.

## The lane

`ticket-singular-lean`, codex `gpt-6-astra` high, `keri:2` pane `%1546`,
root `/tmp/ms-keri-1/ticket-singular-lean`, START 11:12:35Z. Three slices,
each one PR merged and released: (0) land #408 / PR #441 after a fresh grok
audit; (1) `glm` commit owner — Singular as a pinned Lake dependency with a
binding check shown to fail; (2) `muse` commit owner — the keri registry
defined over Singular's `State`/`step`/`foldItems`/`resolve`, `Cage.lean`
retired or reduced, every registry theorem re-proved. Muse and GLM share the
Pi harness and may not alternate against each other, which is why codex holds
the ticket seat. Two design questions are reserved for the desk: how keri's
three leaf states map onto Singular's `active | over` plus application datum,
and which `Cage.lean` divergence theorems survive.

The Lean-invariants epic no longer owns #408; it stays paused.

# 2026-09-15 evening — #408 landed on main; the dependency slice paused mid-candidate

## Landed

**PR #441 merged as `main@0e638fa` at 15:26Z** by the lane's guard-merge:
ruling 12 in the registry machine, the six theorems plus the universal
`pending_go_preserved` (over reachable states, `ReachFar` and horizon as
explicit hypotheses, the arbitrary-state retract fixture kept as the control
that shows why), W3 split at its two real boundaries, the scenario sources
moved to the adopted lifecycle and the corpus regenerated (15 stories, 115
steps), axiom table corrected, mutants relabelled honestly (2 real kills, 4
classes owed, 5 historical rows not claimed). Fresh grok audit 13/0/0 on the
candidate; final head rebased conflict-free. Slice 0 spend 58/60 cheap,
5/6 expensive. Three desk answers governed it (A-001 … A-003).

**Not released.** The release-plan run for `0e638fa` failed exactly as #422
predicts: stale release PR #311 blocks the planner. Every merge from here is
unreleased until #422 lands. Plan v3 row A1; still unruled.

## Slice 1 — the Singular dependency — paused by the operator at 17:23Z

Issue #446, PR #447 (draft, final re-cut after #442/#443's gate was audited
red twice, 6/12 then 7/13 rows). GLM owner (`%1582`) committed candidate
`0bf9bcd`, six product paths, gate green locally; independent grok audit
(`%1584`, write-idle) not launched; nothing pushed to #447. Lane journal
`PAUSED` at 17:23:35Z; wakes only on an explicit release naming
`ticket-singular-lean`. The lane's own CI watcher may still send a pointer;
it is instructed to ignore it.

## Still parked at the desk

- The operator's dormant ruling, (A) minimal dormant checkpoint UTxO or
  (B) a Singular-side value — gates slice 2 (muse). Delivered as an inbox
  note when it comes.
- Plan v3's five rulings; #422.
