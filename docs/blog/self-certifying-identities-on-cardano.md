...
pub type CheckpointDatumV1 {
  ...
  cur_threshold: Threshold,      // integer or fractionally weighted, multi-clause
...
Registration therefore has two gates, and it is the second one that matters. The first gate verifies that the supplied event really derives the claimed identifier — public information, and an attacker can satisfy it. The second gate requires the registration itself to be **signed by the very keys the inception event declares**: the keys written into the checkpoint must equal the event's own key list, and the registration message must carry signatures meeting the event's own threshold. The event is public; the private keys behind the keys it names are not. Registering someone else's identifier would require signing with *their* keys — squatting collapses to key theft, the same standard as every other operation in the design. And because each identifier can only ever be registered once, this gate is load-bearing: a successful squat would lock the true owner out permanently, which is exactly why the keys-must-match rule is a hard obligation of the validator, not an optional courtesy.
...
A normal key rotation with the same witness set needs the post-rotation key-state seal and that set's threshold receipts. A witness-set change uses a two-seal handoff: while the outgoing set is still authoritative, it receipts a pre-announcement committing to the incoming `(witnesses, toad)`; the incoming set then receipts activation. This prevents the controller from choosing the very witness set that is supposed to validate its own introduction. Even an explicit downgrade to `toad = 0` must be endorsed by the outgoing set while it is still in place. Afterward the checkpoint visibly advertises the weaker witnessless mode, so applications may refuse it.
...
One engineering fact makes these rules a first-milestone obligation. A Plutus validator's script hash is fixed at deployment: mandatory receipt checks, the two-seal handoff, freeze, and narrow conviction cannot be bolted on later without migrating every registered identity. They ship in V1, or V1 does not make the claimed safety guarantee.

## The limits we accepted

Every choice above purchases a limitation. Here they are, without cushioning:

- **No delegated identifiers — which excludes less than it sounds.** In the vLEI ecosystem, KERI delegation is an *issuer-infrastructure* pattern: GLEIF's internal identifiers and the QVIs they authorize are delegated. The identities that actually act on-chain — legal entities, role holders, individual builders — are not required to be delegated, and in current practice are not: their business authority comes from the ACDC credential chain, which links credential to credential, not identifier to identifier. None of the seven vLEI credential schemas contains a delegation field at all. V1 therefore serves the acting population and rejects delegated inceptions until a versioned delegation-proof protocol exists — a boundary we monitor, and would move earlier the day a real counterparty shows up delegated.
- **Genesis verification is capped at one Blake3 chunk — 1024 bytes.** We measured this cap against production event logs rather than guessing. A five-key, five-witness organizational inception weighs about 1017 bytes and fits; single-signer and two-key group inceptions fit with room to spare. The only real-world event we found above the line is GLEIF's own internal seven-key Root inception, at 1181 bytes — an identifier that, by the ecosystem's own design, never registers on anyone's chain. Boards of six or more keys with a full witness pool will be served later, by composing chunk proofs across transactions or by the native builtin.
...
- **Freeze and narrow conviction enforcement** — both evidence submission paths are permissionless; lag/recoverable-conflict freeze is not bounty-paid by default, while a proved irreconcilable V1 conviction moves the existing token to a tombstone and pays the prover;
...
What remains *in flight* for the identity-core milestone is the validator itself — including witness-receipt verification, the two-seal handoff, and the evidence paths — tracked in the same repository's milestones. When the next installment of this series appears, its demo will be runnable from that repo.
