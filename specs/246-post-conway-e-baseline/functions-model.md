...
|---|---|---|---|---|
| F-A1 | flake attribute producing the compatibility audit runner | locked upstream pin identities; tracked bridge source set; pinned upstream Lean packages | an executable taking no arguments | Takes no blueprint, program, or evidence input. Rejects any argument. Pin identities are read from the locked inputs, never passed in as literals. |
