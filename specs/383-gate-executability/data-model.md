...
| D383-05 | campaign result | atom total/killed, theorem total/reached/killed, identity result, wrong-reason count, blocked count, stopping reason, pre/post status, exact unique axiom identities and `sorryAx` count |
...
| D383-08 | portable file-set digest | sorted repository-derived relative filename plus content hash; equal at direct and symlink paths, unequal after a member byte mutation |
| D383-09 | axiom identity account | requested qualified name, observed qualified name, uniqueness and exact-set result; cardinality cannot substitute for identity |
| D383-10 | extension disposition | extension number, audit submission, finding class and stop/continue result; same-class submission-2 finding must stop |
