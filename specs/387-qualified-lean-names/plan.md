...
source of theorem identities. Derive the proof-only inversion class by joining
`Step.<constructor>_iff` identities to constructors parsed from the Lean `Step`
inductive; do not maintain a list. Reconcile its 13 members exactly with that
constructor-derived denominator. Reconcile the remaining 74 declarations
exactly with checker rows, lamps and falsifiers, retaining one lamp per
observable declaration. Extend selftest with identity and class-boundary
controls and leave Lean/model/story inputs untouched.
...
2. Commit an executable RED bundle for R387-01 through R387-03, including
   inversion substitution, removal and invalid-entry boundary controls.
...
The change alters parsing, classification and reconciliation failure reporting
only. It classifies but does not close the 13-theorem simulator-observability
gap. It acquires no resource, starts no thread, changes no synchronization
primitive and removes no degradation path. File-read/setup failures must remain
distinguishable from identity or boundary mismatches.
