...
**CESR self-cert verification.** Cardano cannot verify the CESR AID self-certifying property. The CESR AID (`blake3(cesr_inception_event)`) is stored in KeyState as controller-asserted metadata — the registrant supplies it at inception and the script stores it without checking the Blake3 derivation (no Plutus Blake3 builtin exists). The CESR self-cert is an off-chain guarantee only.
...
cardano-aid borrows the pre-rotation primitive from KERI (Key Event Receipt Infrastructure). It does not implement KERI. Specifically, there are no:
