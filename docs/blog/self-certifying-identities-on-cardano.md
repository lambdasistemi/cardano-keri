...
| Convict | Non-fabricable irreconcilable fork proof | Any live #116 role -> TOMBSTONE; ACTIVE pays `D_reg+B` to the convictor, ARMED pays `D_reg` to the convictor and `B` to its hunter, FROZEN pays `D_reg` to the convictor; min-ADA and token remain | Terminality requires a real fork, not transaction racing. |
...
!!! warning "Staged implementation boundary"
    The current #116 branch is deliberately not deployable: it opens only Arm, Claim, and Convict. #114 opens Register with `B`; #115 opens ordinary Advance, including ARMED response and FROZEN thaw; #117 opens CLOSING and Close. Register, every Advance path, and Close remain fail closed in #116 staging, so this final lifecycle is a ratified design, not a claim that held transitions are live.
