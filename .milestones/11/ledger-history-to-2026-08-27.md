...
### Gate version ruled by the operator; the new gate tested before it was written (12:43–12:49Z)

`Q-MS12-004` was answered by the **operator directly**: the project owner's pane sat frozen on
an interactive selection dialog, showing the identical frame an hour apart, and could not rule.
The desk reported it, sent one push naming the pane and the key, did **not** press into that
pane and did **not** route around it to the machine owner. A level was skipped; recorded as an
exception with its cause.

Granted, zero build, ledger `5/5`: preserve the tool-generated report bytes; move the 94
accepted lines to a sibling no generator writes; regenerate `MEASURED-SOURCES-v2.tsv` from its
canonical recipe; correct one planning reference; **version the gate**, retaining frozen v4 as a
defect witness, with the new append-remeasurement check proven able to fail in both directions —
marker absent, and marker naming a stale `source_blob` — and frozen before any product write.

Then the lane put the desk's own dispatch question to the gate it was about to author — *could a
perfect candidate pass this?* — and found the answer was still **no**. `check_flake_discipline`
(`r1-event-key-v4.sh:199-204`) compares the whole non-`onchain/` changed-path set against
`flake-input-declaration-v2.tsv`; desk-verified today as 11 = 11 with an empty symmetric
difference, so the mandated new file would arrive undeclared and go RED. It stopped before
authoring v5 or waking the commit owner.

A **sixth** product path was granted on milestone authority — the declaration itself — and named
as an extension of the operator's five rather than absorbed into them, because without it the
granted work is impossible rather than merely harder. The alternative the lane named was
