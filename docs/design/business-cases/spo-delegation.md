...
- **The SPO** is a legal entity holding a vLEI Legal Entity credential
  (QVI-issued). Its identity anchor is its **pool cold key**: the binding
  ...
  re-attest).
- **The delegator** is the buyer: an institution (fund, treasury, exchange
...
!!! warning "Honest limit — delegation is sticky"
    ...
    publishes a new certificate. The gate guarantees "was identified at
    delegation time," and only with the keeper/eviction extension
...
pitched as **KERI witness hosts** — one vLEI onboarding sells both ("witness
hosting under a verifiable legal identity"), and Amaru bundling is the
distribution vector. **Smallest pilot**: one QVI-credentialed SPO + one
institutional delegator + the pools registry on preprod; no DeFi protocol
needed, no per-trade throughput, exercises L1+L2+L3 end-to-end.
