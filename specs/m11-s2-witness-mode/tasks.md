...
- [ ] **T001** Executable RED for the whole contract: both named examples
...
- [ ] **T002** `M12TxB` with one plan type and one construction entry point taking an explicit
      carriage mode; production selection defined in terms of it. (S2W-R1, S2W-I1)
- [ ] **T003** INLINE rendering preserved as a live path over the same plan, executed in the
      focused suite, serialized size measured. (S2W-R2, S2W-I2)
- [ ] **T004** Pinned protocol quantities read and cited with provenance — package, tarball
      digest, snapshot/genesis. (S2W-R4, S2W-I3)
- [ ] **T005** Reference-script fee derived from the pinned parameters and probed at
      25,599 / 25,600 / 25,601 / 25,617 / 26,448, with the stride boundary revealed. (S2W-R5, S2W-I4)
- [ ] **T006** Per-program **signed** creation-transaction envelope measured, with its own limit,
      that limit's provenance, and a verdict derived from the measurement — reported separately
      from the aggregate. (S2W-R6, S2W-I5)
- [ ] **T007** Aggregate reported as the sum of the measured program sizes, never asserted
      independently. (S2W-I6)
- [ ] **T008** Manifest carries the archived-RED Surface-B object and its evidence hash; no
      `surface_b_sha` shape, argument or validator survives from the seed. (S2W-R7, S2W-I7)
- [ ] **T009** S0 binary-content digest control carried and agreeing with the candidate's own
      `scripts/s0/measure-family.sh`; that script is not edited. (S2W-R8, S2W-I8)
- [ ] **T010** Residuals `A3-F1` (ADVISORY) and `F1-ERROR-CLASSIFICATION` (OPEN, verbatim
      classification) labeled in both manifest and report. (S2W-R9, S2W-I9)
- [ ] **T011** `WITNESS-MODE-REPORT.md` states the determination, the two limits, the fee-tier
      result, the caveats and the residuals — with no inherited number. (S2W-R10)
- [ ] **T012** Shared `testPParams` gains the pinned fee parameter against its then-current shape;
      every existing reader stays green.
- [ ] **T013** Final re-integration of then-current `origin/main`, every byte-dependent baseline
