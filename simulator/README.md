...
| `page-template.mjs`, `page-template/` | the template tool and the stored assets (`page-template.html`, `page.css`, `page-skeleton.html`, `page-ids.txt`) extracted from this page, the reference instance; `check` is a step of the scenario gate, so the page and the template stay one identity. The same assets are the `lean-simulations` skill's. |
...
| `../lean/mutants/run.sh`, `../lean/mutants/mutants.txt` | the mutation campaign over the Lean theorems: the runner applies each mutant of the table by exact text replacement, runs its own identity control first (which must survive), discounts the two structural theorems by name in the log, and writes `../lean/CHECKPOINT-MUTANTS.md` (table and raw log) from that run. |
