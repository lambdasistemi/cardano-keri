...
|---|---|---|
| 1 | ⛔ **PARKED — `OMNIA PAUSA` 2026-08-24T14:20Z, human operator.** The milestone is stopped, not blocked on a technical dependency. | a later written `RELEASE` naming that order. Silence is not authority |
| 1b | ⛔ The gate cannot yet evaluate the accepted candidate: three successive versions were each unsatisfiable, most recently because two composed oracles make **contradictory** demands on the same file — one requires a marker present, the other requires it absent. | a ruling on whether a superseded inherited check may be demoted, recorded and hash-bound, rather than skipped |
...
- 2026-08-24T12:43Z **The gate contradiction was ruled and the repair dispatched.** With the project owner's seat frozen on an interactive dialog and unable to answer for two hours, the operator ruled directly. A level was skipped and is recorded as an exception with its cause, not smoothed over. Granted: a zero-build re-cut that preserves the tool-generated report bytes exactly, moves the accepted hand-authored contracts to a sibling file no generator writes, regenerates the measured-source manifest from its canonical recipe, corrects one planning reference, and versions the gate — **retaining the frozen one as a defect witness** and requiring the new check to be proven able to fail in both directions before it is trusted. Merge, push, CI, acceptance and any further build remain withheld.

- 2026-08-24T12:49Z **The new gate was tested before it was written.** Asked to put *could a perfect candidate pass this?* to the gate it was about to author, the lane found the answer was still no: a generic changed-path-set check would go RED on the very file the ruling mandates, because that file would be undeclared. It stopped before authoring or waking anyone. One further path was granted — the declaration itself — and the alternative was refused: weakening the check to ignore the new file would convert a measured committed-tree input into an unregistered exception, which is making a check pass by making it blind. Declaring a file you deliberately added is the check working.

- 2026-08-24T14:05Z **A withheld push happened, authorized in error by this desk.** Acting on an instruction to finish, the desk authorized push/CI/merge minutes before the ruling that withheld them arrived; the withdrawal landed about two minutes after the push completed. `origin/ms11/r1-event-key-v2` moved to the accepted candidate as a non-force fast-forward and PR #305's head moved with it, still draft. **`main` was never touched** and no merge occurred. Three `pull_request` workflows fired automatically. The containment ruling let the run finish — its outcome grants nothing — refused any rollback, since a force-push would be a second unauthorized act on the same surface, and froze that surface. The cause was named and turned into a standing contract: **a fence check is bound to a surface, not to a moment; re-derive the trigger table for the exact target surface before any remote write.** The desk had derived that table correctly for a different surface hours earlier and failed to carry it across.

- 2026-08-24T14:1xZ **Two more gate versions, both unsatisfiable, both caught before spending the one authorized build.** v7 was frozen and its own self-test went RED on a checker that greps its own source for a forbidden literal, so it rejects every copy of itself. Its log recorded that the inherited oracles were *not reached* — and past that stop point, the desk's independent derivation found the blocking defect: v7 composes two oracles that contradict each other on the same file, one requiring the append marker present in the generated report and the other requiring it absent. The candidate has none there, by design; that split is the whole slice. No tree can satisfy both. A v8 draft repaired the two mechanical faults and was deliberately **not frozen**; the composition question went upward. Every superseded gate is retained byte-identical as a defect witness.

- 2026-08-24T14:20Z **`OMNIA PAUSA`, human operator.** In flight at the stop: a v8 draft, unfrozen, with its composition question open; one authorized build **unspent**; the remote surface frozen; `main` untouched. Nothing was killed, reset or discarded. What releases it: a later written `RELEASE` naming that order.
