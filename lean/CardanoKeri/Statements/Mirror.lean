...
* one UTxO per KERI registry, opened by the seal walk on the registry's
  ...
  issuer AID, the registry id and the root over revoked credential SAIDs;
...
* **the gate** reads one absence per link of a chain; absence in the mirror
  ...
  enforced (#398);
* a revocation pushed under a provisional walk and later superseded stays:
  the mirror **over-revokes and fails closed**.
...
the checkpoint machine lands them) and the mirror's two redeemers. -/
...
def stepFn (p : Params) (env : TelEnv) (s : Sys) : Action → Option Sys
  ...
  | .open issuer w =>
      ...
      | none, some c =>
          if w.tel.kind = .vcp ∧ w.tel.ri = w.tel.i ∧ (sealWalk p env c w.tel.i w).isSome then
            some (s.setReg w.tel.i (some ⟨issuer, w.tel.i, fun _ => false⟩))
          else none
      | _, _ => none
  | .push w =>
