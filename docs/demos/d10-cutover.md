# Preprod cutover rehearsal — 2027-03-07 target

**D-10 story.** As the deployment operator, I want to rehearse the new registry and validator cutover against an inventory of existing identities, so that I can identify unsafe state before touching preprod.
...
Given the exact candidate release, the old preprod manifest and the inventory required by #279; when the team checks deployed script identities, proposed new manifests, parameters and each existing identity’s cutover disposition; then the rehearsal names every mismatch and blocks cutover for an ARMED or FROZEN checkpoint or an unaccounted identity.
...
The #279 identity inventory, old/new manifest diff, candidate archive hash, deployed script verification and disposition for every identity.
...
**Tracking:** [KERI preprod cutover #328](https://github.com/lambdasistemi/cardano-keri/issues/328), [inventory #279](https://github.com/lambdasistemi/cardano-keri/issues/279).
